...
Matrix createLocalK(int e,mesh &m){
    ...
    float u_bar,nu,rho1,rho2,Ae,J,D;
    ...
    productRealMatrix(u_bar*J/(24*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,Beta,2,2,6),6,2,6),matrixA);
    ...
    rho2 = m.getParameter(DENSITY2);
    calculateBPrima(BPrima);
    productRealMatrix(2*rho2*J/(3*D),productMatrixMatrix(g_matrix,productMatrixMatrix(Alpha,BPrima,2,2,3),6,2,3),matrixG);

    //Preparando matrixH (En clase conocida simplemente como H)
    rho1 = m.getParameter(DENSITY1);
    productRealMatrix(rho1/(8*D),productMatrixMatrix(Betat,productMatrixMatrix(Alphat,productMatrixMatrix(Alpha,BPrima,2,2,3),2,2,3),6,2,3),matrixH);
...
Vector createLocalb(int e,mesh &m){
    ...
    float f_x = m.getParameter(EXTERNAL_FORCE);
    float niu = m.getParameter(NIU);
